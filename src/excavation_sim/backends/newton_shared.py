...
class NewtonSharedToolWorld(NewtonToolWorld):
    ...
    def _add_robot(self, builder):
        ...
        for name, x in zip(self.agent_ids, (-0.09, 0.09), strict=True):
            body = builder.add_body(
                xform=wp.transform(wp.vec3(x, 0, 0.30), wp.quat_identity()), label=name
            )
            builder.add_shape_box(
                body, hx=0.04, hy=0.06, hz=0.04, cfg=newton.ModelBuilder.ShapeConfig(density=1200.0)
            )
            bodies.append(body)
    ...
    def _command_forces(self, commands):
        ...
        for name in self.agent_ids:
            ...
            target = np.clip(
                commands[name].velocity_m_s,
                -self.config.velocity_limit_m_s,
                self.config.velocity_limit_m_s,
            )
    ...
    def agent_loads(self) -> dict[str, dict]:
        ...
        return {
            name: {
                "actuator_force_n": self._agent_actuators[body].tolist(),
